defmodule Taskweft.Iso8601Duration do
  @moduledoc """
  ISO 8601 duration parser.  Faithful Elixir port of
  `lean/Planner/Iso8601Duration.lean`.

  Canonical order Y → Mo → D → T → H → Mi → S, each unit at most once.
  Fractions are allowed on the lowest-order component only and only when no
  smaller unit follows.  Weeks must stand alone.  UTC-elapsed normalisation:
  Y = 365 d, Mo = 30 d, W = 7 d; civil time is handled separately in
  `Taskweft.Temporal`.
  """
  ...
  def parse("P"), do: {:ok, []}
  def parse(<<?P, rest::binary>>), do: parse_components(rest, [], false, false, -1, false)
  ...
  defp unit_milliseconds(:s), do: 1_000
  ...
  defp parse_components(<<?T, _::binary>>, _acc, _in_time, true, _last_rank, _frac),
  ...
  defp classify_component(whole, milli, unit_c, rest, acc, in_time, saw_t, last_rank) do
    case lookup_unit(unit_c, in_time) do
      ...
      {:cross, unit} ->
        if in_time, do: {:error, {:date_after_t, unit}}, else: {:error, {:time_before_t, unit}}
  ...
  defp take_int(<<d::utf8, rest::binary>>, acc, raw) when d in ?0..?9,
    do: take_int(rest, acc * 10 + (d - ?0), raw <> <<d::utf8>>)

  defp take_int(rest, acc, raw), do: {acc, raw, rest}

  defp take_frac(<<d::utf8, rest::binary>>, acc, n, raw) when d in ?0..?9 and n < 3,
    do: take_frac(rest, acc * 10 + (d - ?0), n + 1, raw <> <<d::utf8>>)

  defp take_frac(rest, acc, n, raw), do: {acc * pow10(3 - n), raw, rest}
